			...
			<div class="col-lg-10 col-md-12 col-sm-12 col-12">
				...
				<div>
					...
					<ul class="uk-breadcrumb" style="margin-top: 20px;">
						<li style="color: #666;">
							<a href="{{'/eshop'}}" style="color: #666;">Home</a>
							<span class="text-uppercase"> &nbsp;&nbsp; / &nbsp;&nbsp; {{$brandname->brand_name}}</span>
						</li>
					</ul>
				...
				<div class="col-sm-12 col-12 p-0">
					<div class="row" id="showproduct-130">
						...
						@if(isset($brand))
						@foreach($brand as $brandProductview)

						<div class="col-lg-3 cl-md-4 col-sm-6 col-6 mt-4" >
							<div class="homeproducts border">
								
								@if(isset($brandProductview->discount_price))
								<span class="mark" style="margin-left: -18px;">{{$brandProductview->discount_price}} %</span>
								@else
								<span class="mark1" style="margin-left: -18px;"></span>
								@endif

								<center>
									<a href="{{url('product/'.$brandProductview->id)}}"><img src="{{ url($brandProductview->image) }}" class="img-fluid" style="z-index:1; "></a>
								</center>
								<div>
									<a href="{{url('product/'.$brandProductview->id)}}"><center>
										@php
										$content = substr($brandProductview->product_name,0,20);
										@endphp
										{!! $content !!}<br>
										<span>&nbsp;&nbsp;BDT {{$brandProductview->sale_price}}</span></center></a>
									</div>
								</div>
							</div>
							@endforeach
